...
if __name__ == '__main__':
    ...
    artifact_uris = {artifact.get('uri') for result in list(search_xml) for artifact in result.findall('artifact') if
    ...
    for parent_process_uri in parent_process_uris:
        ...
        artifact_uris.append(process.find('input-output-map').find('input').get('uri'))

    artifacts = clarity.batch_get_xml('artifacts', artifact_uris)
    container_uris = {artifact.find("location").find('container').get('uri') for artifact in artifacts}
