...
has '_plate_name_value_mapping' => (
    ...
    isa     => 'HashRef',
    lazy    => 1,
    builder => '_build__plate_name_value_mapping',
  );
sub _build__plate_name_value_mapping {
  my ($self) = @_;

  my %plate_name_value_map = map {
    $_->plate_full_name => $_->barcode;
  } @{$self->epp->plates};
