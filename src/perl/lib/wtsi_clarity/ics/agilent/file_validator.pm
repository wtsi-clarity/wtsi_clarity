...
sub _build_files_by_well {
  ...
  foreach my $file (@{$self->files}) {
    my $index_of_start = $self->_find_index($file->{'start_well'});
    my $index_of_end = $self->_find_index($file->{'end_well'});

    for ($index_of_start..$index_of_end) {
      my $well = $self->wells->[$_];
...
sub _build_files {
  ...
  # Sorted the files by start_cell... needed for validation later and is just handy...
...
sub _find_index {
  my ($self, $well_name) = @_;
  return first_index { $_ eq $well_name } @{$self->wells};
}
...
sub _validate_wells_sequential {
  ...
  foreach (0..($number_of_files - 2)) {
    ...
    my $start_well_on_next_file = $self->wells->[$index_of_end_well + 1];
