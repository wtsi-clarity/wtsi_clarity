...
has ['barcode', 'element_name', 'bed_name'] => (
  is => 'ro',
  isa => 'Str',
  lazy_build => 1,
);
...
sub _build_barcode {
  my $self = shift;
  return $self->bed_elem->textContent;
};

sub _build_element_name {
  my $self = shift;
  ## no critic(ValuesAndExpressions::RequireInterpolationOfMetachars)
  return $self->bed_elem->findvalue('@name');
};
...
sub _build_bed_name {
  ...
  if($self->element_name =~ /(Bed .+)[(].*[)]/sxm) {
    $bed_name = trim $1;
    return $bed_name;
  } else {
    croak 'Could not extract bed name from ' . $self->element_name;
  }
}
...
=head2 element_name Returns the name of the element
