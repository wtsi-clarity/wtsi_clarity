...

{
  ...
  lives_ok {
    $doc = $s->_create_placements_doc
  } 'placement doc created';
  lives_ok {
    $s->_stamping($doc, $stamping_method_ref, $well_calc_ref)
  } 'individual placements created';
}

{
  ...
  lives_ok {
    $s->_analytes
  } 'got all info from clarity (no container type name)';
  my @containers = keys %{$s->_analytes};
...

{
  ...
  lives_ok {
    $s->_analytes
  } q{got all info from clarity ('ABgene 0800')};
  my @containers = keys %{$s->_analytes};
...

{
  ...
  lives_ok {
    $s->_analytes
  } q{got all info from clarity ('ABgene 0765', 'ABgene 0800')};
  my @containers = keys %{$s->_analytes};
  is (scalar @containers, 1, 'one input container, control tube is skipped');
  is (scalar(map {
    $_ =~ /\Ahttp/
...

{
  ...
  lives_ok {
    $doc = $s->_create_placements_doc
  } 'Can create placements doc';
  lives_ok {
    $output_placements = $s->_stamping($doc, $stamping_method_ref, $well_calc_ref)
...

{
  ...
  throws_ok {
    $s->calculate_destination_wells('I:1')
  } qr/Source plate must be a 96 well plate/,
