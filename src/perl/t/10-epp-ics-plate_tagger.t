...
use_ok ('wtsi_clarity::epp::ics::plate_tagger');

throws_ok {
  wtsi_clarity::epp::ics::plate_tagger->new(
...
my $epp = wtsi_clarity::epp::ics::plate_tagger->new(
  process_url => 'http://some.com/processes/151-12090',
  validate    => 1
);
isa_ok( $epp, 'wtsi_clarity::epp::ics::plate_tagger');
is($epp->barcode, 'ABCD1234', 'Gets the tag plate barcode correctly');
lives_ok { $epp->run } 'runs validation';

$epp = wtsi_clarity::epp::ics::plate_tagger->new(
  process_url => 'http://some.com/processes/151-12090',
  validate    => 1
);
