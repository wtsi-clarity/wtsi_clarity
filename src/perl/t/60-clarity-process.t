...

{
  ...
  my $epp = Test::MockObject::Extends->new(
    wtsi_clarity::epp->new(
      process_url => $base_uri . '/processes/24-64174'
    )
  );

  my $xml = XML::LibXML->load_xml(location => $ENV{'WTSICLARITY_WEBCACHE_DIR'} . 'process2.xml');

  my $process = wtsi_clarity::clarity::process->new(xml => $xml, parent => $epp);

  my $expected_workflow_uri = $base_uri . '/configuration/workflows/653';

  is($process->get_current_workflow_uri(), $expected_workflow_uri, 'Returns the expected workflow uri');
}
