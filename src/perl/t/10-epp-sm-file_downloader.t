...
local $ENV{'WTSICLARITY_WEBCACHE_DIR'} = 't/data/epp/sm/file_downloader';
local $ENV{'SAVE2WTSICLARITY_WEBCACHE'} = 0;

use_ok('wtsi_clarity::epp::sm::file_downloader', 'can use wtsi_clarity::epp::sm::file_downloader');

{
  my $file_downloader = wtsi_clarity::epp::sm::file_downloader->new(
      process_url   => $base_uri . '/processes/bla-bla',
      process_type  => 'process_type',
      file_name     => 'Tecan File',
      udf_name      => 'Tecan File',
  );
  isa_ok($file_downloader, 'wtsi_clarity::epp::sm::file_downloader');
  can_ok($file_downloader, qw/ run /);
}
...
{ # Tests for determine the Lims ID of the Output Artifact
  ...
  my @actual_output_limsids = $file_downloader->_output_artifact_uris;
...
{ # Tests for getting the sample limsid
  ...
  is($file_downloader->_sample_limsid, $expected_sample_limsid,
...
{ # Test for getting the ResultFile limsid by the sample limsid and process type
  ...
  my $expected_result_file_limsid = '92-168452';
  is($file_downloader->_result_file_limsid, $expected_result_file_limsid,
...
{ # Tests for getting the associated file's limsid
  ...
  my $expected_file_limsid = '92-168452-40-1183';
  is($file_downloader->_file_limsid, $expected_file_limsid,
...
{ # Tests for exception, when file limsid could not be found
  ...
  throws_ok { $file_downloader->_get_and_validate_file_id }
