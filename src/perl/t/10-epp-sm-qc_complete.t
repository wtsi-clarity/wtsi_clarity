...

{
  ...
  lives_ok { $qc_complete_step->_fetch_and_update_samples($qc_complete_step->process_doc, 'today') } 'managed to fetch and updates the samples';

  cmp_ok(scalar keys %{$qc_complete_step->_samples}, '==', 11,
    'There should be 11 artifacts (Test Fixture).');

  my $QC_COMPLETE_DATE_PATH = q( /smp:sample/udf:field[@name='QC Complete'] );
  my $nb_of_today = 0;
  
  foreach my $sampleURI (keys %{$qc_complete_step->_samples})
  {
    my @elements = util::xml::find_elements( $qc_complete_step->_samples->{$sampleURI}, $QC_COMPLETE_DATE_PATH) ;
    cmp_ok(scalar @elements, '==', 1, 'The completion date should be present on the sample.');
    my $element = shift @elements;
    $nb_of_today += 1 if ($element->textContent eq 'today');
  }

  cmp_ok($nb_of_today, '==', 9, 'Not all the date should be updated.');
}
