...
use_ok('wtsi_clarity::epp::sm::pico_analyser');

local $ENV{'WTSI_CLARITY_HOME'}= q[t/data/config];
my $config = wtsi_clarity::util::config->new();
my $base_url = $config->clarity_api->{'base_uri'};

local $ENV{'WTSICLARITY_WEBCACHE_DIR'} = 't/data/epp/sm/pico_analyser';
...

{
  ...
  isa_ok($pa, 'wtsi_clarity::epp::sm::pico_analyser');
  can_ok($pa, qw/run/);
}
...
# Extract file url / file name
{
  ...
  my $output_analyte = $parser->parse_file('t/data/epp/sm/pico_analyser/GET/artifacts.92-10870?state=5320');
...
# Extract file location
{
  ...
  my $parser = XML::LibXML->new();
  my $file = $parser->parse_file('t/data/epp/sm/pico_analyser/GET/files.92-10870-40-178');
...
{ # _update_output_artifacts
  ...
  my $testdata_path = q(./t/data/epp/sm/pico_analyser/);
