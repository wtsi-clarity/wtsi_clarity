...
{ # _get_legend_content
  ...
  foreach my $datum (@test_data) {
    my ($i,$j, $c, $r) = @{$datum->{'in'}};
    my $expected = $datum->{'out'};
    my $val = wtsi_clarity::epp::generic::worksheet_attacher::_get_legend_content($i,$j, $c, $r);
...
{ # _get_table_data
  ...
  my ($table, $prop) = wtsi_clarity::epp::generic::worksheet_attacher::_get_table_data($TEST_DATA->{'output_container_info'}->{'container_uri'}->{'container_details'}, 5,6);
...
{ # _get_legend_properties
  ...
  foreach my $datum (@expected_data) {
    ...
    my $prop = wtsi_clarity::epp::generic::worksheet_attacher::_get_legend_properties($TEST_DATA, $i, $j, 1, 1);
